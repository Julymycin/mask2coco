...
def main(args):
    ...
    for idx, image_name in enumerate(train_imgs):
        if image_name.endswith(('jpg', 'png')):
            ...
            mask_temp = np.zeros((image.size[1], image.size[0]), dtype=np.uint8)
            ...
            # # MHP v2 foot area
            # mask_temp = np.where((mask_temp >= 20) & (mask_temp <= 33) &
            #                      (mask_temp != 30) & (mask_temp != 31), 1, 0)
            # MHP v1 foot area
            # mask_temp = np.where((mask_temp == 9) | (mask_temp == 10), 1, 0)
